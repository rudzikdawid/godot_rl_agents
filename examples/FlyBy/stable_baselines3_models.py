...
parser.add_argument(
    "--resume_models_config",
    default=None,
    type=str,
    help="The path to a json config with mapped group names to models paths"
         "Use this to infer from a saved models.",
)
parser.add_argument(
    "--save_model_path",
    default=None,
...
parser.add_argument(
    ...
    help="Instead of training, it will run inference on a loaded model for --timesteps steps. "
         "Requires --resume_model_path or --resume_models_config to be set."
)
parser.add_argument(
    "--viz",
    action="store_true",
...
if args.inference and ((args.resume_model_path is None) and (args.resume_models_config is None)):
    raise parser.error("Using --inference requires --resume_model_path or --resume_models_config to be set.")
...
def remap_dict(original_dict):
    remapped_dict = {}
    for key, value in original_dict.items():
        path_zip = pathlib.Path(value)
        print("Loading model: " + os.path.abspath(path_zip))
        remapped_dict[key] = PPO.load(path_zip, env=env, tensorboard_log=args.experiment_dir)
    print('remapped_dict', remapped_dict)
...
if args.resume_models_config:
    print('args.resume_models_config: ', args.resume_models_config)
    print('pathlib.Path: ', pathlib.Path(args.resume_models_config))
    with open(args.resume_models_config) as file:
        config = json.load(file)
        remap_config = remap_dict(config)
        print('remap_config', remap_config)
        print('config: ', config, type(config), list(config.items()), list(config.keys()), list(config.values()))

if args.resume_model_path is None:
    print('111 experiment_dir: ', args.experiment_dir)
    model = PPO("MultiInputPolicy", env, ent_coef=0.0001, verbose=2, n_steps=32, tensorboard_log=args.experiment_dir)
else:
    print('222 experiment_dir: ', args.experiment_dir)
...
if args.inference:
    ...
    if args.save_checkpoint_frequency is None:
        print('no save_checkpoint_frequency')
